...
@section('content')
    <div class="container mt-4">
        <h1 class="text-dark mb-4">Edit Pelanggan</h1>
        <form action="{{ route('pelanggan.update', $pelanggan->id) }}" method="POST" class="bg-light p-4 rounded shadow-sm">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="email" id="email" class="form-control"value="{{ $pelanggan->email }}"
                    required>
            </div>

            <div class="mb-3">
                <label for="telepon" class="form-label">Telepon</label>
                <input type="number" name="telepon" id="telepon" class="form-control"value="{{ $pelanggan->telepon }}"
                    required>
            </div>

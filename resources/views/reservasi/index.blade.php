...
@section('content')
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="text-primary">Daftar Reservasi</h1>
        ...
        <form method="GET" action="{{ route('reservasi.index') }}" class="mb-4">
            <div class="input-group">
                <input type="text" name="search" id="search" class="form-control"
                       value="{{ request('search') }}" placeholder="Cari berdasarkan nama pelanggan, meja, tanggal, status">
            ...
            <tbody>
                @foreach ($reservasis as $reservasi)
                    <tr>
                        ...
                        <td><span{{ $reservasi->status == 'confirmed' ? 'success' : ($reservasi->status == 'pending' ? 'warning' : 'danger') }}">{{ ucfirst($reservasi->status) }}</span></td>
                        <td>
